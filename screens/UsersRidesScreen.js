...
const UsersRidesScreen = props => {
  ...
  const [hostsEvents, setHostsEvents] = React.useState(false);
  ...
  return (
    ...
    >
      <SimpleStyleScrollView
        ...
        style={StyleSheet.applyWidth(
          { paddingLeft: 24, paddingRight: 24, paddingTop: 72 },
          dimensions.width
        )}
      >
          ...
          handlers={{
            onData: fetchData => {
              const handler = async () => {
                try {
                  const events = (
                    await SupabaseEventsApi.getEvents$DescendingOrder$GET(
                      Constants,
                      { select: '*' }
                    )
                  )?.json;
        ...
        >
          {({ loading, error, data, refetchGetEvents$DescendingOrder$ }) => {
            ...
            return (
              <>
                <SimpleStyleSectionList
                  data={filterEventsByUserId(fetchData, Constants['user_id'])}
                          ...
                          <Touchable>
                            ...
                            >
                              ...
                              >
                                ...
                                >
                                  {formatDate(sectionListData?.date)}
                                  {', '}
                                  {formatHours(
                                    sectionListData?.start_time
                                  )}{' '}
                            ...
                            {/* Meetup Location View */}
                            <>
                              {sectionListData?.event_status ===
                              'Previous' ? null : (
                                <View
                                  style={StyleSheet.applyWidth(
                                    {
                                    ...
                                    {Constants['loading'] ? null : (
                                      <Button
                                        ...
                                        onPress={() => {
                                          try {
                                            Linking.openURL(
                                              `https://www.google.com/maps?q=${sectionListData?.meetup_lat},${sectionListData?.meetup_lon}`
                                            );
                                          } catch (err) {
                                            console.error(err);
                                          }
                ...
                >
                  <>
                    {hasData(
                      filterEventsByUserId(fetchData, Constants['user_id'])
                    ) ? null : (
                      <Text
                        accessible={true}
                        selectable={false}
        ...
        <SupabaseEventsApi.FetchGetEventByHostIdGET
          handlers={{
            onData: fetch2Data => {
              try {
                setHostsEvents(hasEvents(fetch2Data));
              } catch (err) {
                console.error(err);
              }
            },
...
export default withTheme(UsersRidesScreen);
